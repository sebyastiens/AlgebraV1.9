...
library DataStorage {
  ...
  function _getAverageTick(
    ...
    uint32 oldestTimestamp = IAlgebraPool(poolAddress).timepoints(oldestIndex).blockTimestamp;
    int56 oldestTickCumulative = IAlgebraPool(poolAddress).timepoints(oldestIndex).tickCumulative;
    ...
    if (lteConsideringOverflow(oldestTimestamp, time - WINDOW, time)) {
      if (lteConsideringOverflow(lastTimestamp, time - WINDOW, time)) {
        ...
        Timepoint storage startTimepoint = IAlgebraPool(poolAddress).timepoints(index);
        ...
        Timepoint memory startOfWindow = getSingleTimepoint(poolAddress, time, WINDOW, tick, index, oldestIndex, 0);
  ...
  function binarySearch(
    ...
    do {
      beforeOrAt = IAlgebraPool(poolAddress).timepoints(uint16(current)); // checking the "middle" point between the boundaries
      ...
      if (initializedBefore) {
        if (lteConsideringOverflow(timestampBefore, target, time)) {
          ...
          atOrAfter = IAlgebraPool(poolAddress).timepoints(uint16(current+1)); // checking the next point after "middle"
  ...
  function getSingleTimepoint(
    ...
    if (secondsAgo == 0 || lteConsideringOverflow(IAlgebraPool(poolAddress).timepoints(index).blockTimestamp, target, time)) {
      Timepoint memory last = IAlgebraPool(poolAddress).timepoints(index);
      if (last.blockTimestamp == target) {
        return last;
      } else {
        // otherwise, we need to add new timepoint
        int24 avgTick = int24(_getAverageTick(poolAddress, time, tick, index, oldestIndex, last.blockTimestamp, last.tickCumulative));
        int24 prevTick = tick;
        {
          if (index != oldestIndex) {
            Timepoint memory prevLast;
            Timepoint storage _prevLast = IAlgebraPool(poolAddress).timepoints(index-1); // considering index underflow
    ...
    require(lteConsideringOverflow(IAlgebraPool(poolAddress).timepoints(oldestIndex).blockTimestamp, target, time), 'OLD');
    ...
    )
  {
    ...
    for (uint256 i = 0; i < secondsAgos.length; i++) {
      current = getSingleTimepoint(poolAddress, time, secondsAgos[i], tick, index, oldestIndex, liquidity);
  ...
  function getAverages(
    ...
    uint16 oldestIndex;
    Timepoint storage oldest = IAlgebraPool(poolAddress).timepoints(0);
    uint16 nextIndex = index + 1; // considering overflow
    if ( IAlgebraPool(poolAddress).timepoints(nextIndex).initialized) {
    ...
    Timepoint memory endOfWindow = getSingleTimepoint(poolAddress, time, 0, tick, index, oldestIndex, liquidity);

    uint32 oldestTimestamp = oldest.blockTimestamp;
    if (lteConsideringOverflow(oldestTimestamp, time - WINDOW, time)) {
      Timepoint memory startOfWindow = getSingleTimepoint(poolAddress, time, WINDOW, tick, index, oldestIndex, liquidity);
      return (
  ...
  function initialize(
    ...
    require(!IAlgebraPool(poolAddress).timepoints(0).initialized);
    IAlgebraPool(poolAddress).timepoints(0).initialized = true;
    IAlgebraPool(poolAddress).timepoints(0).blockTimestamp = time;
    IAlgebraPool(poolAddress).timepoints(0).averageTick = tick;
  }
  ...
  function write(
    ...
    Timepoint storage _last = IAlgebraPool(poolAddress).timepoints(index);
    ...
    uint16 oldestIndex;
    // check if we have overflow in the past
    if (IAlgebraPool(poolAddress).timepoints(indexUpdated).initialized) {
      oldestIndex = indexUpdated;
    }

    int24 avgTick = int24(_getAverageTick(poolAddress, blockTimestamp, tick, index, oldestIndex, last.blockTimestamp, last.tickCumulative));
    int24 prevTick = tick;
    if (index != oldestIndex) {
      Timepoint storage _prevLast = IAlgebraPool(poolAddress).timepoints(index - 1); // considering index underflow
    ...
    IAlgebraPool(poolAddress).timepoints(indexUpdated) = createNewTimepoint(last, blockTimestamp, tick, prevTick, liquidity, avgTick, volumePerLiquidity);
  }
}
